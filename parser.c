...
**  files; it started with Argify and SynTable as a cute ten-minute
    ...
    REGISTER char	 *av[];
{
    ...
    if (ac == 6 && EQ(av[0], "-f") && EQ(av[2], "-a")
     && (EQ(av[4], "!=") || EQ(av[4], "-ne")))
	return(GetStat(Name = Expand(av[1])) && Ftype(Name) == F_FILE
	       && EQ(Expand(av[3]), Expand(av[5])) ? !V : V);
