...
#ifdef	RCSID
static char RCS[] =
	"$Header: /Users/yaya/ccshar/RCS/makekit.c,v 1.1 2005/08/03 02:31:07 root Exp root $";
#endif	/* RCSID */
...
static char	TEMP[22];		/* Temporary manifest file	*/
#ifdef	MSDOS
static char FLST[22];				/* File with list for shar	*/
#endif	/* MSDOS */
    ...
    char		*av[];
{
    ...
    Notkits = FALSE;
    Iscsh = FALSE;
    while ((i = getopt(ac, av, "1ceh:i:k:n:mo:ps:t:x")) != EOF)
	switch (i) {
	default:
	    Fprintf(stderr, "usage: makekit %s\n        %s\n",
		    "[-1] [-c] [-e] [-x] [-k #] [-s #[k]] [-n Name] [-t Text]",
